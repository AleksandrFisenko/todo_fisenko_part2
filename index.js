(() => {
  ...
  async function checkAllTasks(status) {
    await fetch(
      (BASE_URL + PATH + `?isChecked=${status}`),
      {
        method: 'PUT',
      }
    )
  ...
  const handleClick = (event) => {
    ...
    if (clickedItemName === "todo-checkbox") {
      editTaskById(taskId, event.target.nextElementSibling.value, event.target.checked)
      render()
    }
  ...
  const saveTask = (event) => {
    if (isTextValid(event.target.value)) {
      const taskId = event.target.parentElement.id
      const replacedValue = replaceSymbols(event.target.value.replace(/ +/g, ' ').trim())
      console.log(event.target.nextElementSibling.textContent, event.target.value)
      if(event.target.nextElementSibling.textContent !== event.target.value)
        editTaskById(taskId, replacedValue, event.target.previousElementSibling.checked)
    } else render();
  }
  ...
  const render = () => {
    ...
    renderArray.forEach(task => {
      ...
      <li class="todo-item" id="${task.id}">
          ...
          <input class="todo-input" maxlength="256" hidden value="${task.text}"/>
